...
# V4L2 formats
V4L2_PIX_FMT_YUYV = 0x56595559  # YUY2 format
...
def set_video_format(fd, width, height, format_code=V4L2_PIX_FMT_YUYV):
    """Configure the video format on the device"""
    fmt = v4l2_format()
    fmt.type = V4L2_BUF_TYPE_VIDEO_OUTPUT
    fmt.u.fmt.pix.width = width
    fmt.u.fmt.pix.height = height
    fmt.u.fmt.pix.pixelformat = format_code
    fmt.u.fmt.pix.field = V4L2_FIELD_NONE
    fmt.u.fmt.pix.sizeimage = width * height * 2  # YUY2 format
    
    try:
        fcntl.ioctl(fd, VIDIOC_S_FMT, fmt)
        print(f"Video format set: {width}x{height} format=0x{format_code:08x}")
        return 0
