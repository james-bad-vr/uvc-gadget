...
def subscribe_event(fd, event_type):
    ...
    sub.type = event_type
    sub.id = 0
    # Add the necessary flags for proper event subscription
    ...
    try:
        ...
        # Debug: print the subscription structure
        sub_bytes = bytes(sub)
...
def handle_event(fd):
    ...
    try:
        ...
        if event.type == UVC_EVENT_SETUP:
            ...
            # Debug: dump the event structure
            event_bytes = bytes(event)
            print(f"Raw event data: {' '.join(f'{b:02x}' for b in event_bytes[:32])}")
    except Exception as e:
